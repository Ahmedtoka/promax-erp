<?php

/** الفريق الميداني: العهدة، الزيارات، التسليم، الريفيل. */
return [

    // ---------- العهدة ----------
    'van_stock' => 'العهدة',
    'load_van' => 'تحميل عهدة',
    'close_van_stock' => 'قفل العهدة',
    'loaded' => 'المحمّل',
    'sold' => 'المباع',
    'returned' => 'المرتجع',
    'remaining' => 'المتبقي',
    'custody_not_enough' => 'العهدة ناقصة :short وحدة من :product.',
    'custody_closed' => 'عهدة النهارده مقفولة خلاص.',
    'no_custody_today' => 'مفيش عهدة محمّلة النهارده.',

    // ---------- الزيارات ----------
    'visit' => 'زيارة',
    'visits' => 'الزيارات',
    'check_in' => 'تشيك إن',
    'check_out' => 'تشيك أوت',
    'open_visit' => 'عندك زيارة مفتوحة بالفعل.',
    'duration' => 'المدة',

    // ---------- التسليم ----------
    'purchase_order' => 'أمر توريد',
    'purchase_orders' => 'أوامر التوريد',
    'deliver' => 'تسليم',
    'arrive' => 'تسجيل وصول',
    'must_arrive_first' => 'سجّل الوصول الأول قبل التسليم.',

    // ---------- الريفيل ----------
    'merchandising_visit' => 'زيارة رف',
    'shelf' => 'الرف',
    'shelf_before' => 'الرف قبل',
    'shelf_after' => 'الرف بعد',
    'branch_store_room' => 'مخزن الفرع',
    'moved_to_shelf' => 'اتنقل للرف',
    'out_of_stock' => 'ناقص خالص',
    'photos_required' => 'لازم صورة الرف قبل وبعد.',
    'moved_exceeds_store' => 'مينفعش تنقل أكتر من اللي في مخزن الفرع.',

    'replenishment' => 'ريفيل',
    'replenishment_request' => 'طلب ريفيل',
    'assign_to_rep' => 'تنزيل على مندوب',
    'awaiting_dispatch' => 'مستني التوزيع',

    // ---------- التراكينج ----------
    'tracking' => 'التراكينج',
    'activity' => 'التحركات',
    'journey_plan' => 'خط السير',
    'coverage' => 'التغطية',
    'zone' => 'الزون',

    // ---------- رسائل فلو الأبلكيشن ----------
    'must_check_out_first' => 'لازم تعمل تشيك أوت من :client الأول.',
    'must_close_visit_first' => 'لازم تقفل زيارة :client الأول.',
    'visit_already_closed' => 'الزيارة مقفولة أصلاً.',
    'visit_has_replenishment' => 'الزيارة دي فيها طلب ريفيل بالفعل.',
    'photo_before_required' => 'لازم تصوّر الرف قبل الريفيل.',
    'photo_after_required' => 'لازم تصوّر الرف بعد الريفيل.',
    'moved_exceeds_store_qty' => 'مش ممكن تنقل :moved من :product ومخزن الفرع فيه :store بس.',
    'cash_only_client' => 'العميل ده تحصيل فوري — البيع كاش بس.',

    // ---------- مسميات حقول الفاليديشن ----------
    'attr_place_name' => 'اسم المكان',
    'attr_place_photo' => 'صورة المكان',
    'attr_official_docs' => 'الأوراق الرسمية',
    'attr_shelf_photo' => 'صورة الرف',

    // ---------- أحداث التايم لاين ----------
    'event_check_in' => 'تشيك إن — :client',
    'event_check_out' => 'تشيك أوت — :client',
    'event_visit_minutes' => 'مدة الزيارة :minutes دقيقة',
    'event_invoice' => 'فاتورة :number — :client',
    'event_app_open' => 'فتح الأبلكيشن',
    'event_return' => 'مرتجع من :client',
    'gift_new_request' => 'طلب جديد',
    'gift_not_your_request' => 'الطلب ده مش من تسجيلك.',
    'event_arrived' => 'وصول — :client',
    'event_delivered' => 'تسليم :number — :client',
    'event_delivered_sub' => ':qty وحدة • :amount ج',
    'event_client_request' => 'طلب عميل جديد — :name',
    'event_awaiting_manager' => 'مستني موافقة المدير',
    // المدير بيفتح الأكاونت بنفسه — بيتوافق في نفس اللحظة (١١ أغسطس ٢٠٢٦)
    'event_client_auto_approved' => 'اتفتح واتوافق فوراً — المدير فتحه بنفسه',
    'event_merch_visit' => 'زيارة رف — :client',
    'event_replenishment' => 'طلب ريفيل :number — :client',
    'event_qty_requested' => ':qty قطعة مطلوبة',
    'event_refill' => 'ريفيل — :client',
    'event_refill_sub' => ':moved قطعة اتنقلت للرف • :short صنف ناقص',
    'event_exit' => 'خروج — :client',
    'event_extra_load' => 'تحميل إضافي على العربية',
    'event_loaded_by' => 'من :name',

    // ---------- الإشعارات ----------
    'notif_client_approved_title' => 'تمت الموافقة على :name',
    'notif_client_approved_body' => 'العميل اتضاف لزونك — تقدر تعمله فاتورة.',
    'notif_client_review_title' => 'طلب :name تحت المراجعة',
    'notif_client_review_body' => 'العميل هياخد وقت للمراجعة.',
    'notif_client_rejected_title' => 'طلب :name اترفض',
    'notif_client_rejected_body' => 'مينفعش تعمل فواتير للعميل ده.',
    'notif_replenishment_pending_title' => 'طلب ريفيل :number مستني موافقتك',
    'notif_replenishment_pending_body' => ':client — :qty قطعة — من :user',
    'notif_replenishment_rejected_title' => 'طلب :number اترفض',
    'notif_replenishment_rejected_body' => 'راجع المدير بتاعك.',
    'notif_replenishment_new_title' => 'طلب ريفيل جديد :number',
    'notif_replenishment_new_body' => ':client — :amount ج',
    'notif_replenishment_assigned_title' => 'طلبك :number اتنزّل على :name',
    'notif_replenishment_assigned_body' => 'هيتجهّز من المخزن قريب لـ :client.',
    // فلو الريفيل الجديد (١٥/٨) — موافقة ← تجهيز ← استلام في العهدة
    'notif_rpl_approved_title' => 'طلب الريفيل :number اتوافق عليه',
    'notif_rpl_approved_body' => ':client — نزل المخزن يتجهّز بأمر :pick.',
    'notif_rpl_ready_title' => 'طلب الريفيل :number اتجهّز',
    'notif_rpl_ready_body' => 'استلمه من المخزن بأمر التجهيز :pick.',
    'notif_rpl_received_title' => 'طلب الريفيل :number دخل عهدتك',
    'notif_rpl_received_body' => ':qty قطعة زادت في العربية.',
    'notif_po_new_title' => 'أمر توريد جديد :number',
    'notif_po_rejected_title' => 'الحسابات رفضت :number',
    'notif_po_edited_title' => 'الحسابات عدّلت :number',
    'notif_po_new_body' => ':client — :amount ج',
    'notif_po_assigned_title' => 'اتنزّل عليك أمر توريد :number',


    // ═══ تسليم العهدة والهدايا ═══
    'handout' => 'تسليم عهدة',
    'handout_note' => 'إذن تسليم عهدة',
    'pick_rep' => 'اختر المندوب',
    'qty_sale' => 'للبيع',
    'qty_gift' => 'هدايا',
    'do_handout' => 'سلّم العهدة',
    'total_units' => 'الإجمالي',
    'carrier_note' => 'ملاحظة',
    'carrier_note_ph' => 'رقم العربية أو أي ملاحظة',
    'awaiting_receipt' => 'بانتظار الاستلام',
    'received' => 'تم الاستلام',
    'awaiting_hint' => 'خرجت من المخزن والمندوب لسه مااستلمش',
    'issued_at' => 'خرجت',
    'handed_at' => 'اتسلمت',
    'handout_history' => 'تسليمات اتمّت',
    'handout_history_hint' => 'كل تحميلات العربيات: مين وإمتى وبكام — والورقة بتتطبع تاني في أي وقت',
    'handout_value' => 'القيمة',
    'handout_value_note' => 'القيمة الموضحة بأسعار :list — للاسترشاد فقط، والعهدة بتتحاسب بالقطعة.',
    'issued_by' => 'سلّمها',
    'handout_done' => 'إذن :number اتعمل — :rep. اطبع الورقة وخلّيه يمضي.',
    'handout_warning' => 'ده طلب تجهيز — البضاعة مش هتخرج من الأرفف غير لما المخزن يأكد التجهيز من شاشة «تجهيز الطلبات». بعد التأكيد بيوصل إشعار للمندوب يستلم من الأبلكيشن.',
    'load_requested' => 'طلب التجهيز :number اتعمل — :rep. اطبع الورقة وسلّمها للمخزن.',
    'send_to_prep' => 'إرسال للتجهيز',
    'search_product_ph' => 'اكتب اسم المنتج أو الكود… (أو دوس هنا يفتح الكل)',
    'select_all' => 'اختيار الكل',
    'unselect_all' => 'إلغاء التحديد',
    'no_selected_hint' => 'ابحث فوق واختار الأصناف — كل اختيار بينزل صف هنا بكميته',
    'preparing_title' => 'تحت التجهيز',
    'preparing_hint' => 'طلبات اتبعتت للمخزن ولسه ماتأكدتش — التأكيد من شاشة تجهيز الطلبات',
    'go_prepare' => 'تجهيز',
    'not_a_field_role' => 'الحساب ده مش من فريق الشارع (مندوب / سواق / بروموتر / مدير) — العهدة بتتحمّل على فريق الشارع بس.',
    'gift_notice' => 'فيه :qty وحدة هدايا في العهدة دي. مش للبيع — والمندوب لازم يسجّل اداها لمين.',
    'handout_pledge' => 'أقر أنا المندوب الموقّع أدناه باستلام البضاعة الموضّحة أعلاه بالكميات والباتشات المدوّنة، وبمسؤوليتي عنها لحين تسليم حصيلة البيع والمرتجعات وقفل العهدة.',
    'sign_issuer' => 'اللي سلّم',
    'sign_rep' => 'المندوب المستلم',
    'no_open_custody' => 'مفيش عهدة مفتوحة النهارده.',
    'gift_not_enough' => 'الباقي :left بس من الهدايا.',
    'gifts' => 'الهدايا',
    'gift_left' => 'الباقي',
    'gift_given' => 'اتوزّع',
    'record_handout' => 'سجّل التوزيع',
    'track_gift' => 'هدية :qty × :product',

    // ═══ سايكل المرتجعات (٨ أغسطس ٢٠٢٦) ═══
    'return_policy' => 'طريقة المرتجع',
    'return_policy_cash' => 'كاش فوري',
    'return_policy_account' => 'خصم من حسابه',
    'return_policy_exchange' => 'تبديل ببضاعة',
    'return_policy_credit_next' => 'رصيد المرة الجاية',
    'return_policy_cash_hint' => 'المندوب بيرد القيمة نقدي دلوقتي',
    'return_policy_account_hint' => 'القيمة بتقلل مديونيته',
    'return_policy_exchange_hint' => 'بيتاخد بضاعة بدالها في نفس الزيارة',
    'return_policy_credit_next_hint' => 'رصيد ليه بيتخصم من التوريد الجاي',
    'return_policy_pick' => 'اختار طريقة المرتجع الأول',
    'return_cond' => 'الحالة',
    'return_cond_good' => 'سليم',
    'return_cond_damaged' => 'تالف',
    'return_cond_hint' => 'السليم بيرجع للبيع، والتالف بيتسلّم للمخزن لوحده',
    'return_returnable' => 'المتاح للرد',
    'return_none_returnable' => 'العميل ده مالوش مشتريات متاحة للرد',
    'return_exchange_next' => 'دلوقتي اعمل فاتورة بالبضاعة البديلة',
    'returns' => 'المرتجعات',
    'return_doc' => 'مستند مرتجع',
    'return_saved' => 'اتسجّل المرتجع :number',
    'return_good_units' => 'سليم',
    'return_damaged_units' => 'تالف',
    'return_entry_posted' => 'القيد اتكتب في كشف الحساب: :memo',

    // ═══ أوبشنات الزيارة الجديدة (2026-08-09) ═══
    'attr_collect_amount' => 'مبلغ التحصيل',
    'attr_collect_proof' => 'صورة الإثبات',
    'event_collect' => 'حصّل :amount من :client',
    'notif_collect_title' => 'تحصيل غير نقدي :amount',
    'notif_collect_body' => ':method من :client — بواسطة :user. راجع الإثبات وطابقه.',
    'event_shelf_before' => 'صوّر الرف قبل الترتيب عند :client',
    'event_shelf_after' => 'صوّر الرف بعد الترتيب عند :client',
    'shelf_photos' => 'صور ترتيب الرفوف',
    'event_goods_request' => 'طلب بضاعة :number لـ :client',
    'notif_goods_request_title' => 'طلب بضاعة جديد :number',
    'notif_goods_request_body' => ':client — :qty قطعة، طلبها :user من عند العميل. راجع ووافق.',
    'notif_goods_request_wh_title' => 'طلب بضاعة في السكة :number',
    'notif_goods_request_wh_body' => ':client — :qty قطعة. أمر التجهيز هينزل أول ما الموافقة تتم.',
    // ═══ لوكيشن العميل من الأبلكيشن (١٤ أغسطس ٢٠٢٦) ═══
    'event_set_location' => 'ضبط لوكيشن :client',
    'notif_rep_location_title' => 'لوكيشن عميل اتظبط من الأبلكيشن',
    'notif_rep_location_body' => 'المندوب :user ضبط لوكيشن :client وهو قدام المحل — راجعه في شاشة تأكيد اللوكيشن.',
    'notif_client_request_title' => 'طلب عميل جديد: :name',
    'notif_client_request_body' => 'قدّمه :user ومستني موافقتك.',
    'replenishment_origin_rep' => 'طلب مندوب عند العميل',
    'replenishment_origin_promoter' => 'طلب بروموتر من زيارة رف',

    // ═══ بورد عهد المناديب (١٠ أغسطس ٢٠٢٦) ═══
    'vans_open' => 'عربيات مفتوحة',
    'vans_street_value' => 'قيمة البضاعة في الشارع',
    'vans_units_left' => 'وحدات متبقية',
    'vans_no_custody' => 'من غير عهدة',
    'vans_value_hint' => 'القيم بسعر البيع — الباقي معروض بكل قايمة أسعار مفعّلة، والتصفية بالقطع',
    'van_status' => 'الحالة',
    'van_open' => 'مفتوحة',
    'van_closed' => 'مقفولة',
    'van_none' => 'مفيش عهدة',
    'vans_loaded' => 'المحمّل',
    'vans_sold' => 'اتباع',
    'vans_returned' => 'مرجّع',
    'vans_gifts' => 'هدايا معاه',
    'vans_left' => 'الباقي معاه',
    'vans_progress' => 'نسبة التصريف',
    'vans_now_at' => 'دلوقتي عند',
    'vans_sales_today' => 'مبيعات النهارده',
    'vans_expiring' => ':count صنف قرب ينتهي',
    'vans_close' => 'قفل العربية',
    'vans_close_confirm' => 'قفل عهدة :name؟ بعد القفل مايقدرش يبيع منها، وأول استلام جديد يفتح عهدة جديدة.',
    'notif_po_reapproval_title' => 'أمر :number اتعدّل ورجع للموافقة',
    'notif_po_reapproval_body' => ':client — عدّله :by، محتاج اعتماد من جديد',
    // ═══ إلغاء التسليم (١١ أغسطس ٢٠٢٦ مساءً) ═══
    'event_po_aborted' => 'رجّع أمر :number — :client من غير تسليم',
    'notif_po_aborted_title' => 'أمر :number اترجع من غير تسليم ⛔',
    'notif_po_aborted_body' => ':client — :rep لغى التسليم: :reason',
    'notif_forced_out_title' => 'الإدارة قفلت زيارتك ⚠️',
    'notif_forced_out_body' => 'زيارتك عند «:client» اتقفلت بواسطة :by — لو لسه شغال هناك اعمل تشيك إن من جديد.',

    // ═══ بورد مبيعات المناديب + البورد المدموج (١٢ أغسطس ٢٠٢٦) ═══
    'sales_sub' => 'فلوس المندوب = فواتيره + أوامر التوريد المسلَّمة (من القيود) + تحصيلاته الميدانية',
    'sales_kpi_cash' => 'إجمالي كاش',
    'sales_kpi_credit' => 'إجمالي آجل',
    'sales_kpi_cash_coll' => 'تحصيلات نقدية',
    'sales_kpi_other_coll' => 'تحصيلات غير نقدية',
    'sales_kpi_other_coll_hint' => 'تحويل / شيك / فيزا',
    'sales_kpi_refunds' => 'مرتجعات كاش',
    'sales_invoices' => 'فواتير',
    'sales_cash' => 'مبيعات كاش',
    'sales_credit' => 'مبيعات آجل',
    'sales_incl_pos' => 'منها أوامر توريد: :v',
    'sales_coll_cash' => 'تحصيل نقدي',
    'sales_coll_transfer' => 'تحويل / محفظة',
    'sales_coll_cheque_card' => 'شيك + فيزا',
    'sales_refunds' => 'مرتجعات كاش',
    'sales_net' => 'صافي النقدية',
    'sales_net_hint' => 'مبيعات الكاش + التحصيل النقدي − مرتجعات الكاش',
    'board_sub' => 'العهدة والفلوس والحركة لكل مندوب في نظرة واحدة',
    'board_kpi_working' => 'شغالين دلوقتي',
    'board_kpi_sales' => 'مبيعات الفترة',
    'board_kpi_colls' => 'تحصيلات الفترة',
    'board_attendance' => 'الحضور',
    'board_custody' => 'العهدة',
    'board_sales_col' => 'المبيعات',
    'board_colls_col' => 'التحصيلات',
    'board_cash' => 'كاش',
    'board_credit' => 'آجل',
    'board_noncash' => 'غير نقدي',
    'board_visits' => 'الزيارات',
    'board_visits_hint' => 'اتقفلت / الكل',
    'board_last_event' => 'آخر حركة',

    // ═══ تصحيح إداري للعهدة (١٢ أغسطس ٢٠٢٦) ═══
    'custody_adjust' => 'تعديل العهدة',
    'custody_adjust_hint' => 'صحّح عد اتسجّل غلط: اكتب المحمَّل الصح لكل صنف بالقطع. دي تسوية سجل بس — المخزن مش بيتلمس نهائياً (فرق المخزن الحقيقي مكانه جرد المخزن). ومينفعش تنزل تحت المتصرّف فعلاً (مباع + مرجّع + متحوّل).',
    'custody_adjust_reason' => 'سبب التعديل',
    'custody_adjust_reason_ph' => 'مثال: التحميل اتسجّل 100 والصح 10',
    'custody_adjust_loaded' => 'المحمَّل الحالي',
    'custody_adjust_new' => 'المحمَّل الصح',
    'custody_adjust_floor' => 'الحد الأدنى',
    'custody_adjust_gift_new' => 'الهدايا الصح',
    'custody_adjust_total_value' => 'إجمالي قيمة المحمَّل الجديد',
    'custody_adjust_floor_err' => 'مينفعش :product ينزل تحت :floor — ده المتصرّف منه فعلاً (مباع + مرجّع/موزّع).',
    'custody_adjust_min_js' => 'أقل رقم مسموح هنا :floor — ده المتصرّف بمستندات فعلاً (مباع + مرتجع + متحوّل). لو العربية فيها أقل من كده، الفرق ده عجز بيتحاسب في التصفية مش من هنا.',
    'custody_adjust_no_change' => 'مفيش أي رقم اتغيّر.',
    'custody_adjust_none' => 'مفيش عهدة مفتوحة للمندوب ده.',
    'custody_adjust_note' => 'تصحيح إداري: :reason',
    // ═══ تفريغ العربية بضغطة (٢٨ أغسطس ٢٠٢٦) ═══
    'clear_btn' => 'تفريغ العربية',
    'clear_title' => 'تفريغ عهدة المندوب',
    'clear_hint' => 'في العربية دلوقتي :units قطعة. التفريغ بيصفّر المتبقي بس — المباع والمرتجع والمحوَّل لمندوب تاني بيفضلوا زي ما هُم في تصفيته.',
    'clear_mode_return' => 'البضاعة ترجع المخزن',
    'clear_mode_return_hint' => 'المندوب رجّع العربية فعلاً — كل كمية بترجع لرفّ باتشها والمخزون بيزيد تاني. ده المسار الصح لأي حركة حقيقية.',
    'clear_mode_wipe' => 'تصفير بلا حركة مخزن (بيانات تجريبية)',
    'clear_mode_wipe_hint' => '🔴 البضاعة دي خرجت من المخزن فعلاً ومخصومة منه — التصفير كده بيخليها تختفي من السيستم من غير أثر. للتجارب بس. البضاعة اللي ضاعت أو تلفت مكانها الجرد.',
    'clear_reason_ph' => 'مثال: تنظيف بيانات تجربة / المندوب رجّع العربية بالكامل',
    'clear_apply' => 'صفّر العهدة',
    'clear_confirm' => 'متأكد إنك عاوز تصفّر كل اللي في العربية؟ العملية دي مالهاش زرار رجوع.',
    'clear_already_empty' => 'العربية فاضية أصلاً — مفيش متبقي يتصفّر.',
    'clear_done' => 'العربية اتفضت: :units قطعة (:mode).',
    'clear_event_body' => ':mode — :units قطعة',
    'event_custody_cleared' => 'تفريغ العهدة بالكامل',
    'notif_custody_cleared_title' => 'عهدتك اتفضت',
    'notif_custody_cleared_body' => ':by فضّى عربيتك بالكامل. السبب: :reason',

    'event_custody_adjust' => 'تعديل إداري على العهدة',
    'notif_custody_adjusted_title' => 'عهدتك اتعدلت إدارياً',
    'notif_custody_adjusted_body' => ':by صحّح أرقام عهدتك — السبب: :reason. افتح شاشة العهدة تشوف الأرقام الجديدة.',

    // ═══ تعديل طلب الريفيل المستني (١٢ أغسطس ٢٠٢٦) ═══
    'notif_replenishment_edited_title' => 'طلبك :number اتعدل',
    'notif_replenishment_edited_body' => 'المدير عدّل أصناف/كميات طلب التوريد لفرع :client قبل التنزيل.',

    // ═══ حارس التكرار في طلب العميل الجديد (١٥ أغسطس ٢٠٢٦) ═══
    'dup_client_found' => 'فيه :count عميل مسجل بنفس الاسم أو التليفون — راجعهم قبل ما تكمّل.',
    'mv_load' => 'تحميل من المخزن',
    'mv_gift' => 'هدية / عيّنة',

    // إشعار إلغاء أمر التوريد (٢١/٨)
    'notif_po_cancelled_title' => 'أمر التوريد :number اتلغى',
    'notif_po_cancelled_wh' => 'أمر :client اتلغى — بضاعته اترجعت للمخزن بمستند تحويل من عربيتك.',
    'notif_po_cancelled_custody' => 'أمر :client اتلغى — بضاعته فضلت في عهدتك، بيعها عادي.',
    'event_po_cancel_wh' => 'بضاعة أمر التوريد :number رجعت المخزن (الأمر اتلغى)',

    // إشعارات المستند اليدوي (٢١/٨)
    'notif_manual_invoice_title' => 'فاتورة :number اتسجلت عليك من المكتب',
    'notif_manual_invoice_body' => 'لعميل :client بإجمالي :amount بتاريخ :date — اتخصمت من عهدتك.',
    'notif_manual_return_title' => 'مرتجع :number اتسجل عليك من المكتب',
    'notif_manual_return_body' => 'من عميل :client بتاريخ :date — البضاعة رجعت لعهدتك.',
    'notif_manual_gift_title' => 'هدية اتسجلت عليك من المكتب',
    'notif_manual_gift_body' => 'لعميل :client بتاريخ :date — اتخصمت من رصيد هداياك.',
    'custody_adjust_more' => '+:n تغيير تاني',

    // جرد الرف وإنهاء الزيارة بدون تصوير (٢١ سبتمبر ٢٠٢٦)
    'attr_no_photo_reason' => 'سبب عدم التصوير',
    'count_dates_reversed' => 'تاريخ انتهاء :product قبل تاريخ إنتاجه — راجع التاريخين.',
    'no_photos_alert_title' => 'زيارة اتقفلت بدون تصوير',
    'no_photos_alert_body' => ':user قفل زيارة :client من غير صور الرف. السبب: :reason',
    'event_no_photos' => 'زيارة :client اتقفلت بدون تصوير',
    'count_not_ready' => 'جرد الرف لسه مش متفعّل على السيرفر — بلّغ الإدارة.',

    // شاشة المرتجعات المطوّرة (٢١ سبتمبر ٢٠٢٦)
    'ret_kpi_value' => 'إجمالي المرتجعات',
    'ret_kpi_docs' => ':count مستند',
    'ret_kpi_rate' => 'نسبة المرتجعات من المبيعات',
    'ret_kpi_rate_hint' => 'من مبيعات الفترة :sales',
    'ret_kpi_rate_pick' => 'حدد فترة عشان النسبة تتحسب',
    'ret_of_units' => 'من القطع',
    'ret_search_ph' => 'رقم المرتجع أو اسم العميل',
    'ret_item' => 'الصنف',
    'ret_condition' => 'الحالة',
    'ret_cond_good' => 'سليم',
    'ret_cond_damaged' => 'تالف',
    'ret_qty' => 'الكمية',
    'ret_unit_price' => 'سعر الوحدة',
    'ret_net' => 'الصافي',
    'ret_tax' => 'الضريبة',
    'ret_docs' => 'مستندات',
    'ret_by_product' => 'أكتر الأصناف رجوعا',
    'ret_by_client' => 'أكتر العملاء مرتجعات',
    'ret_by_rep' => 'المرتجعات بالمندوب',
    'ret_export_docs' => 'تصدير المرتجعات',
    'ret_export_lines' => 'تصدير تفصيلي بالأصناف',
    'ret_period_note' => 'الفترة بتاريخ قيد المرتجع في كشف الحساب — نفس رقم المرتجعات في الصفحة الرئيسية والتقارير.',
];
