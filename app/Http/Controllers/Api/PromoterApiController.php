...
class PromoterApiController extends Controller
{
    ...
    public function bootstrap(Request $request): JsonResponse
    {
        ...
        // ═══ المناطق — بالتسكين الحقيقي (٢٨/٨ — إعادة بناء المنسق) ═══
        //
        // «العملاء دي ظاهرة عنده مش عارف جاية منين وهو مش عليه عملاء
        // أصلاً»: قايمة الفروع القديمة كانت heuristic (زونه + قناته
        // بتجيب كل عملاء الزون). تاب المناطق الجديد بيقرا من
        // `zonesPayload` — **نفس سلسلة الظهور بتاعة المندوب**: زون
        // نشط ومتعلّم له في `zone_user`. اللي مش متسكن عليه حاجة
        // بيشوف فاضي، والتوزيع من «تخصيص العملاء والمناطق» زي أي حد.
        ...
        return response()->json([
            ...
            'journey' => $journey,
            // تاب المناطق — بالتسكين الحقيقي (شرح فوق)
            'zones' => $zones,
            ...
            'products' => Product::where('active', true)->orderBy('code')->get()
                ->map(fn ($p) => [
                    ...
                    'code' => $p->code,
                    'name' => $p->displayName(),
                    // ⚠️ **الصورة مش رفاهية في منتقي الأصناف.** البروموتر
                    // بيقف قدام الرف وبيدوّر على الصنف بشكله مش باسمه —
                    // «بار بروتين ٧٠ جرام» مكرر ٧ مرات بنكهات مختلفة،
                    // والفرق بينهم في الصورة. من غيرها بيقرا ٧ أسطر
