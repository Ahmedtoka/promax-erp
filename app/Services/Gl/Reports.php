...
class Reports
{
    ...
    public function balancesByAccount(?Carbon $from, ?Carbon $to): array
    {
        ...
        foreach ($this->sums($from, $to) as $id => $s) {
            ...
            $node = (int) $id;
            // ⚠️ حارس الحلقة: أب بيشاور على نفسه أو على حفيده (داتا
            // مكسورة) كان هيخلي اللوب مالهاش آخر والصفحة تعلّق من غير
            // رسالة. الوقوف عند عقدة اتزارت قبل كده بيمنع الحلقة **و**
            // بيمنع إضافة نفس المبلغ مرتين لنفس الأب.
            $walked = [];
            while ($node !== null) {
                // أب معلّق (الصف اتمسح) — بنسيب السلسلة في صمت، ده مش
                // خطأ المستخدم ومالوش مكان يتعرض فيه على الشاشة
                if (isset($walked[$node]) || ! array_key_exists($node, $net)) {
                    break;
                }
                $walked[$node] = true;
                $net[$node] += $v;
