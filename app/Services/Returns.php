 ...
 * ═══════════════════════════════════════════════════════════════
 *
 * 1. ⚠️ **المرتجع بيتسعّر من الفاتورة الأصلية، مش بسعر النهارده.**
 *    ده كان باج حقيقي: لو قايمة السعر أو الخصم اتغيّروا بين البيع
 *    والمرتجع، الدفتر بيسيب باقي مالوش تفسير. `allocate()` بتمشي
 *    على فواتير العميل من الأحدث للأقدم وبتاخد من كل سطر المتاح
 *    منه — بسعره هو وضريبته هو.
 *
 * 2. ⚠️ **السقف من نفس المصدر.** «متاح للرد» = كمية السطر ناقص
 *    اللي اترجّع منه قبل كده (`return_items.invoice_item_id`).
 *    مندوب كان يقدر يدّي العميل رصيد بلا حد قبل كده.
 *
    ...
    public static function create(
        ...
        return DB::transaction(function () use (
            ...
            foreach ($wanted as $pid => $byCondition) {
                ...
                // يعدّوا — ونفس البضاعة ترجع مرتين.
                $pool = self::lockedPool($client, $pid);
                $available = array_sum(array_column($pool, 'qty'));
                $asked = array_sum($byCondition);

                if ($asked > $available) {
                    throw new Rejected(__('api.return_over_purchased', [
