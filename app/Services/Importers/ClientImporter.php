    ...
    public function apply(array $rows): array
    {
        ...
        DB::transaction(function () use ($rows, &$created, &$updated, &$opened, &$skipped, &$channels, &$zones, &$groups, &$nameIndex, &$points) {
            foreach ($rows as $row) {
                ...
                //
                // ⚠️ **إلا الصف اللي معاه كود رسمي (٢٤/٨).** شيتات السلاسل
                // (خير زمان K001...) كل فرع فيها بكوده الرسمي — والكود
                // الجديد حكم قاطع إنه فرع مستقل. الإحداثيات هنا أصلاً
                // جايه من جيوكودينج العنوان فبتتلم في مركز الحي، وحارس
                // القرب كان بيرمي 13 فرع من 28 «قريب من موجود» وهما
                // فروع حقيقية جنب محلات تانية. الحارس فاضل زي ما هو
                // للشيتات اللي من غير أكواد — هي اللي بتتكرر بأسماء
                // مختلفة فعلاً.
                $hasOfficialCode = trim((string) $code) !== '';

                if ($existing === null && ! $hasOfficialCode
