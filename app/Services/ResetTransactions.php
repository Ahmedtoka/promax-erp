 ...
 * حركة الموردين، المستحقات، طلبات العملاء الجدد، **الحضور
 * والانصراف، زيارات المخزن، وسجل الأوديت**.
 *
...
class ResetTransactions
{
    ...
    private const ORDER = [
        ...
        // ═══ الحضور وزيارات المخزن واللوجز (اتضافوا 2026-08-08) ═══
        //
        // ⚠️ **الحضور حركة مش إعداد.** بعد التفضية كان المندوب بيفتح
        // الأبلكيشن ويلاقي نفسه «مسجّل حضور من 3 أيام» ويوم مفتوح
        // ماحدش قفله — فالحارس يفتح له كل حاجة على يوم وهمي.
        // ⚠️ **البانشات قبل الأيام** — `attendance_punches.day_id`.
        'attendance_punches', 'attendance_days',

        // ⚠️ **زيارات المخزن حركة كمان.** زيارة مفتوحة سايبة بتخلّي
        // حارس `in.warehouse` مفتوح للأبد بعد المسح.
        'warehouse_visits',

        // ⚠️ **سجل الأوديت.** بيسجّل كل تعديل وكل فتح صفحة — بعد
        // التفضية بيفضل فيه آلاف الصفوف بتشاور على فواتير وعهد
        // اتمسحت، والشاشة بتفتح على أسطر مالهاش هدف.
        'activity_logs',
