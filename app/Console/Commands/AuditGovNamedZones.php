...
class AuditGovNamedZones extends Command
{
     ...
     *
     * ⚠️ **طرق بس — مش معالم** (تصحيح بعد تشغيلة `--fix`). كانت
     * الليستة فيها «مول» و«نادي» و«برج» و«بجوار»، فـ«تراس مول
     * الشروق» اترفض رغم إنه في الشروق فعلاً. الفرق: «شارع/طريق/
     * محور» بتسمّي **خط** ممكن يعدّي على مناطق تانية، لكن «مول/برج/
     * نادي» بتسمّي **نقطة** — والنقطة المسمّاة باسم منطقة بتكون
     * جوّاها في الغالب.
     */
    ...
    /**
     * أسماء بديلة لمناطق موجودة — الاسم اللي المندوب بيكتبه في
    ...
    private const ALIASES = [
        // «التجمع» لوحدها أشهر بكتير من «القاهرة الجديدة» في العناوين.
        // «التجمع الأول/الخامس» مناطق قايمة بذاتها، وقاعدة «أطول
        // مطابقة تكسب» بتخليهم يفوزوا على البديل العام ده.
        'القاهرة الجديدة' => ['التجمع', 'new cairo', 'tagamoa'],
        'مصر الجديدة' => ['هليوبوليس', 'heliopolis'],
        'الرحاب' => ['rehab'],
        'المقطم' => ['mokattam', 'mokatam'],
        // «جنينة مول» و«البطراوي» معالم ما بتتكررش بره مدينة نصر
        'مدينة نصر' => ['nasr city', 'جنينه مول', 'البطراوي'],
        'الزمالك' => ['zamalek'],
        ...
        // ═══ مناطق `promax:seed-zones` بيعملها ═══
        //
        // ⚠️ الصفوف دي **ماتشتغلش قبل ما المناطق تتعمل** — التقرير
        // بيحذّر بأسمائها في أوله لو لسه ناقصة.
        'المنيل' => ['manyal'],
        'المعمورة' => ['elmaamora', 'el maamora'],
        // خطأ إملائي متكرر في العناوين: «الزكية» بالزاي
        'القرية الذكية' => ['القريه الزكيه'],
        ...
        'طريق مصر إسكندرية الصحراوي' => [
            'cairo alex', 'alexandria cairo desert', 'cairo alexandria desert',
            'alex desert', 'desert road', 'الطريق الصحراوي',
            'طريق القاهرة الاسكندرية', 'طريق الاسكندرية الصحراوي',
        ],
    ];
    ...
    private function norm(string $s): string
    {
        ...
        // ⚠️ **فصل الأرقام عن الحروف** — المناديب بيكتبوا «ش90التجمع»
        // و«التجمعD5» من غير مسافة، فـ«التجمع» ماتبقاش كلمة كاملة
        // والمطابقة بتفشل على عنوان واضح تماماً لعين بشرية.
        $s = (string) preg_replace('/(\p{N})(\p{L})/u', '$1 $2', $s);
        $s = (string) preg_replace('/(\p{L})(\p{N})/u', '$1 $2', $s);
    ...
    {
        // ⚠️ **الأربع خانات** — العميل ممكن يكون اسمه العربي فاضي
        // والإنجليزي فيه المنطقة (`EL REHAB 1`)، أو العكس. قراءة
        // `name` و`address` بس كانت بتضيّع عملاء بيانتهم موجودة
        // بالكامل بس في الخانة التانية.
        $addr = $this->norm(implode(' ', [
            (string) $c->address,
            (string) $c->address_ar,
            (string) $c->name,
            (string) $c->name_en,
