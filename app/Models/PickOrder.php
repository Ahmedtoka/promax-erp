        ...
        try {
            DB::transaction(function () use ($receivedByItem, $rep, &$custody, &$variance, $note) {
                ...
                foreach ($this->items as $item) {
                    ...
                    if ($got > 0) {
                        // ⚠️⚠️ **بند عهدة لكل (منتج + باتش + مصدر)** —
                        // المصدر انضم للمفتاح في ١٤ أغسطس ٢٠٢٦.
                        //
                        // قبل كده كان (منتج + باتش) بس، فبضاعة اتحمّلت
                        // لأمر توريد معيّن كانت بتندمج في نفس صف بضاعة
                        // العهدة العادية لو الصنف والباتش واحد — ومحدش
                        // يقدر يقول بعدها «الكراتين دي بتاعة الأمر ولا
                        // بتاعة العهدة». المالك طلب الفرق ده بالنص.
                        //
                        // ⚠️ `purchase_order_id` هو الفيصل مش `purpose`:
                        // طلب الريفيل بيتحوّل PO كمان (`assignTo`)،
                        // فبضاعته بضاعة أمر توريد بنفس المعنى بالظبط.
                        ...
                        $line = $custody->items()->firstOrNew([
                            ...
                            'source' => $source,
                            'source_ref_id' => (int) ($this->purchase_order_id ?? 0),
                        ]);
