...
class Scope
{
    ...
    public static function inZone(?User $rep, ?Client $client, ?array $extraZoneIds = null): bool
    {
        ...
        // ═══ بول الفريق (قرار المالك ١١/٨ مساءً) ═══
        // عميل من بول فريق المندوب (نفس مديره) = منطقته منطقة المندوب
        // بحكم الدوكترين — التخطيط والتسكين على أي عميل من البول من
        // غير احتكاك تشيك بوكس المناطق. ده **مش** تخفيف للحارس: الفحص
        // على `manager_id` أضيق من فحص الزون نفسه (الزون ممكن يبقى
        // فيه عملاء فرق تانية)، و`sameTeam` كان بيسمح بالحالة دي أصلاً.
        if ($client->inPoolOf($rep)) {
            return true;
        }
